#!/usr/bin/env bash
# Evaluate a trained checkpoint on the official TEST split (or val).
#   Usage:  bash scripts/evaluate_segformer.sh
#           CHECKPOINT=/path/to/best SPLIT=test bash scripts/evaluate_segformer.sh
#           bash scripts/evaluate_segformer.sh --num-vis 8     # also save figures
set -euo pipefail
...
SPLIT="${SPLIT:-test}"
echo "Evaluating checkpoint: $CKPT  (split=$SPLIT)"
